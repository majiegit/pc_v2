    ...
    <a-form ref="formRegister" :form="form" id="formRegister">
      ...
      <a-form-item>
        <a-input size="large" :placeholder="$t('user.login.mobile.placeholder')"
                 v-decorator="['phone', {rules: [{ required: true, message: $t('user.phone-number.required'), pattern: /^1[3456789]\d{9}$/ }, { validator: this.handlePhoneCheck } ], validateTrigger: ['change', 'blur'] }]">
      ...
      <a-row :gutter="16">
        ...
        <a-col class="gutter-row" :span="8">
          <a-button
            ...
            :loading="state.smsSendBtnLoading"
...
<script>
  ...
  import {saveRegister, getPhoneVerificationCode} from '@/api/login'
        ...
        validateFields(['phone'], {force: true},
          (err, values) => {
            if (!err) {
              ...
              state.smsSendBtnLoading = true
              getPhoneVerificationCode(values.phone).then(res => {
                this.$message.success(res.message)
